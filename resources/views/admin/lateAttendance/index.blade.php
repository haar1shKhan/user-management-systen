...
@section('breadcrumb')
    <li class="breadcrumb-item">Leave Management / {{ $page_title }}</li>
@endsection
                        ...
                        <div class="d-flex justify-content-between">
                            @if ($trash)
                                ...
                                <div>
                                    ...
                                    <button class="btn btn-danger massActionButton" type="submit"
                                        onclick="setActionType('forceDestroyAll')" data-bs-original-title=""
                                        title="">{{ trans('global.deleteAll') }}</button>
                                    <button class="btn btn-success massActionButton" onclick="setActionType('restoreAll')"
                                        type="submit" data-bs-original-title=""
                                        title="">{{ trans('global.restoreAll') }}</button>
                                </div>
                            @else
                                <h3>Late Attendance History</h3>
                                <div>
                                    {{-- @can('role_delete') --}}
                                    <button class="btn btn-primary" type="button" data-bs-toggle="modal"
                                        data-bs-target=".bd-example-modal-lg">Apply</button>
                                    ...
                                    @if (!empty($lateAttendances))
                                        @foreach ($lateAttendances as $list)
                                            ...
                                            <td>
                                                ...
                                                <td>
                                                    {{ date_format($list->created_at,"d/m/Y"); }}
                                                </td>
                                                <td>
                                                    <span class="font-weight-bold">From: </span> {{ date('h:i a', strtotime($list->from)) }} 
                                                    <span class="font-weight-bold">To: </span> {{ date('h:i a', strtotime($list->to)) }} 
                                                </td>
                                                ...
                                                <td>
                                                    <ul class="action">
                                                        ...
                                                        <form
                                                            action="{{ route('admin.' . $url . '.destroy', ['late_attendance' => $list->id]) }}"
                                                            method="post">
                                                            @csrf
                                                            @method('DELETE')
                                                            <li class="delete"><button class="border-none" type="submit"><i
                                                                        class="icon-trash"></i></button></li>
                                            ...
                                            <div class="modal fade" id="editModal{{ $list->id }}" tabindex="-1"
                                                ...
                                                <div class="modal-dialog modal-dialog-centered" role="document">
                                                    <form
                                                        action="{{ route('admin.' . $url . '.update', ['late_attendance' => $list->id]) }}"
                                                        ...
                                                        <div class="modal-body">
                                                            <div class="d-flex flex-column mx-3">
                                                                <input class="form-control" id="validationCustom01"
                                                                    value="{{ $list->reason }}" name="reason"
                                                                    type="text" required="" data-bs-original-title=""
                                                                    title="">
    ...
    <div class="modal fade bd-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel"
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="myLargeModalLabel">Apply for Late Attendance</h4>
...
@section('script')
    ...
    <script src="https://laravel.pixelstrap.com/cuba/assets/js/notify/notify-script.js"></script>
    <script>
        @if ($errors->any())
...
{{-- 
<form action="{{ route('admin.'.$url.'.update', ['late_attendance' => $list->id]) }}" method="post">
                                                        @csrf
                                                        @method('PUT')
