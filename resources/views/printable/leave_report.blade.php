  ...
  <page size="A4">
    ...
    <div class="header">
      ...
      <div class="col-6">
        ...
        <p class="info"><b>Salary will be stopped during vacation: </b>
            @if ($leave->entitlement->policy->advance_salary === 0)
                No
            @else
                yes
            @endif
    ...
    <hr>
    <div class="body">
        <p class="info">I kindly request you to grant me a {{$numberOfDays}}-day leave as follows:</p>
        <br>
        <p class="info"><b>Leave Type: </b>{{$leave->entitlement->policy->title}}</p>
        <p class="info"><b>From: </b>{{ date('j M Y', strtotime($leave->from)) }}</p>
        <p class="info"><b>To: </b>{{ date('j M Y', strtotime($leave->to)) }}</p>
        <p class="info"><b>Duration: </b>{{$numberOfDays}} Days</p>
        <p class="info"><b>Start date: </b>{{date('j M Y', strtotime($leave->to . ' +1 day'))}}</p>
        <br>
        ...
        <p class="info"><b>Current Balance: </b>{{$days}} Days</p>
    </div>
    <br>
    <br>
    <br>
